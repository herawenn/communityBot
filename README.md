...
2.  **Configuration File (`config.json`):**
    ...
    *   Ensure your Dehashed API key is properly configured in the `apis` section.
...
| Command      | Description                                              | Usage                                       |
| :----------- | :------------------------------------------------------- | :------------------------------------------ |
| `bomber`     | Sends spam emails to a target email address.             | `bomber <email_address> <number_of_emails>` |
...
| `poll`       | Creates a poll with answer options.         | `poll <question> <option1> <option2>` |
| `endpoll`    | Ends a poll given its message ID.           | `endpoll <message_id>`                |
...
The bot provides password cracking functionality using John the Ripper and brute-forcing capabilities using Hydra.

*   **Wordlists:** The effectiveness depends heavily on the wordlists provided. You can customize the wordlist path in `config.json`.
*   **Hash Types:** Use the `[hash_type]` argument (optional) to specify the hash format for John the Ripper. Refer to John the Ripper's documentation for supported hash types.
*   **Brute-force Attacks:** Use the `!brute` command along with the target IP address, port, and service to initiate a brute-force attack using Hydra.
...
The bot allows you to search for leaked data using the Dehashed API via the `!search` command. Provide the query type (e.g., 'email', 'username', 'password') and the query itself to retrieve relevant information.
